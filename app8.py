...
def ensure_state():
    ...
    if "area_config_editor_df" not in st.session_state:
        st.session_state["area_config_editor_df"] = pd.DataFrame([
            {"區域代碼": area, "區域名稱": cfg["name"], "工位數": cfg["positions"], "人力數": cfg["manpower"]}
...
def render_dashboard():
    ...
    panel_open("專案彙總")
    st.dataframe(zh(summary_df), use_container_width=True, height=400, hide_index=True)
    panel_close()

def render_forecast():
    page_header("Forecast輸入", "生管可手動輸入或匯入業務 forecast，系統依工時與產能自動排程。", ["排程", "輸入"])
    ...
    with left:
        ...
        edited = st.data_editor(
            ...
            column_config={
                ...
                "產品型號": st.column_config.SelectboxColumn("產品型號", options=["STK4", "STK5"]),
                "數量": st.column_config.NumberColumn("數量", min_value=1, step=1),
                "預計出貨日": st.column_config.DateColumn("預計出貨日"),
            },
            key="forecast_editor_v8",
        )
    ...
    with right:
        panel_open("排程前檢查")
        st.dataframe(
            st.session_state["forecast_df"].rename(columns={
                "project_no": "專案號",
                "product_type": "產品型號",
                "qty": "數量",
                "due_date": "預計出貨日",
...
def render_schedule_list():
    page_header("排程清單", "查看指定日期各工位實際正在裝配的機台與工序，並與工位圖連動。", ["排程", "清單"])
    if not has_result():
    ...
    with c2:
        products = st.multiselect("產品型號", options=sorted(df["product_type"].unique()), default=sorted(df["product_type"].unique()))
    with c3:
    ...
    show_cols = ["area_name", "slot_no", "machine_no", "product_type", "step_label", "planned_start", "planned_finish", "estimated_ship_date"]
    st.dataframe(zh(filtered[show_cols]), use_container_width=True, height=560, hide_index=True)
    panel_close()

def render_station_board():
    page_header("工位看板", "以工位矩陣方式查看指定日期各站別正在生產哪一台機台。", ["工位", "矩陣"])
    if not has_result():
        st.info("請先到「Forecast輸入」執行排程。")
        return
    ...
    panel_open("工位矩陣")
    pivot = filtered.pivot_table(index=["area_name"], columns="工位代號", values="display", aggfunc="first").reset_index()
    pivot = pivot.rename(columns={"area_name": "區域名稱"})
...
def render_capacity_report():
    page_header("產能負荷報表", "依日期與區域查看有效產能、忙碌工位數與利用率。", ["報表", "產能"])
    if not has_result():
        st.info("請先到「Forecast輸入」執行排程。")
        return
...
def render_delivery_report():
    page_header("出貨達交報表", "給主管看的交期達成率、風險案件與最終完工日期。", ["報表", "出貨"])
    if not has_result():
        st.info("請先到「Forecast輸入」執行排程。")
        return
...
def render_gantt():
    page_header("工序甘特總覽", "用管理視角檢視各工序的開始、完成、所屬工位與交期。", ["報表", "甘特"])
    if not has_result():
        st.info("請先到「Forecast輸入」執行排程。")
        return
    ...
    schedule_df = schedule_df.rename(columns={"task_name": "任務名稱"})
    st.dataframe(zh(schedule_df[["任務名稱", "area_name", "slot_no", "planned_start", "planned_finish", "hours", "due_date", "can_meet_due"]]), use_container_width=True, height=560, hide_index=True)

def render_settings_master():
    page_header("主檔設定", "管理區域工位與人力設定。", ["設定"])
    ...
    with c2:
        if st.button("還原預設值", use_container_width=True):
            ...
            st.session_state["area_config_editor_df"] = pd.DataFrame([
                {"區域代碼": area, "區域名稱": cfg["name"], "工位數": cfg["positions"], "人力數": cfg["manpower"]}
                for area, cfg in DEFAULT_AREA_CONFIG.items()
            ])
            st.success("已還原成預設產能設定。")
    panel_close()
